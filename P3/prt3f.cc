   		...
   		for(int j=0; j<n; j++){
	   			if(rank==0){
	   				for(int k=0; k<size; k++){
	   					MPI_Send(&ms[i+k][j].r, 1, MPI_INT, rank+k, 0, MPI_COMM_WORLD);
	   					MPI_Send(&ms[i+k][j].g, 1, MPI_INT, rank+k, 1, MPI_COMM_WORLD);
	   					MPI_Send(&ms[i+k][j].b, 1, MPI_INT, rank+k, 2, MPI_COMM_WORLD);
	   				}
	   			}
	   			...
	   			if(rank==0){
	   				for(int k=0; k<size; k++){
	   					MPI_Recv(&aux[i+k][j].r,1,MPI_INT,MPI_ANY_SOURCE,k+100,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
	   					MPI_Recv(&aux[i+k][j].g,1,MPI_INT,MPI_ANY_SOURCE,k+101,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
	   					MPI_Recv(&aux[i+k][j].b,1,MPI_INT,MPI_ANY_SOURCE,k+102,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
	   				}
	   			}
   		}
   	}
